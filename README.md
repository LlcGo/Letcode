...
977.有序数组的平方
其中的数组本来就是排序拍好的但是是有负数的，需要将他们都平方之后，还是原来从小到大的顺序
+ 最大数肯定是在左边或者右边的
+ 需要新建一个数组返回，如果还是原来的数组，那么如果是左边的最大，就会覆盖掉右边的值，就会少了需要的数据

209.长度最小的子数组
+ 像滑动窗口一样从头滑动到尾
+ 一个指针负责统计值，每个值相加，加到最后大于等于target，然后就停止
+ 另一个指针在这个终止的范围内取值找到最小的结果返回 右边的指针 - 左边的指针 等于 多少个数相加又因为 是从0开始的 所以还要 + 1

59.螺旋矩阵Ⅱ
+ 我们首先要确认他的循环次数，可以根据他的 长度/2可以得出循环的次数
+ 定义一个loop做为最初的循环次数，每次深入一层循环就要++，之后可以用来作为限制他的遍历范围（俩个作用一个是控制循环次数，一个数控制范围）
+ 最后还需要一个层数的下标 第一层是 0 第二层 刚开始的下标应该是2 所以每次循环完一层之后都要 ++
+ 最后还有可能会出现少填充中间的情况，如果是奇数的话需要填充最后一个

# 链表
203.移除链表
+ 如果需要统一操作删除就加一个虚拟头节点
+ 判断的时候使用当前的cur值来判断不要用next可能会是空指针
...
9/8
123 买卖股票最佳时机Ⅲ
1. 状态多分出俩种，分别是第一次持有，第一次不持有，第二次持有，第二次步持有
